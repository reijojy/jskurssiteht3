...
function printTuotteet() {
    const piilota : string = "Piilota tuotteet";
    const tulosta : string = "Tulosta tuotteet"; 
...
function haeTuotteet() {
    ...
    let x: string = "<table class=\"table-bordered table-hover table-striped\"><tr>";
   ...
   // x += "<th>Ostohinta</th><th>Kate-%</th>";
    tuoteObj.tuotteet.forEach(element => {
        let oh: number = element.ostohinta;
        let mh: number = element.myyntihinta;
        let kate = (oh:number, mh:number): number => { return (1 - (oh / mh)) * 100 };
...
function printLinkit() {
    const piilota : string = "Piilota linkit";
    const tulosta : string = "Tulosta linkit"; 
